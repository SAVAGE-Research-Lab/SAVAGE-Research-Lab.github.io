---
permalink: /projects/
title: "🛠️Technology &amp; Intellectual Property"
author_profile: true
---
Our lab **bridges the gap** between theoretical breakthroughs and industrial-scale impact. The maturity of our research is solidified by an extensive portfolio of **authorized national patents and software copyrights**, which serve as the **technological bedrock** for our secure solutions in **autonomous platforms, low-altitude economy, and privacy preservation**.

---

## I. System Security &amp; Intelligent Anomaly Detection

Focusing on kernel-level protection, multi-threading security, and log-based threat hunting.

- **[Patent] Intelligent Active Software Protection Method** based on mixed control flow graphs of general and kernel functions.
  - Inventors: **Qian Zhou**, Xingxing Zhang, Liang Liu.
  - ID: ZL202111384884.1 (Authorized: Aug 2024)

- **[Patent] Multi-threaded Program Anomaly Detection** based on system call sequences.
  - Inventors: **Qian Zhou**, Kaibin Zhang, Liang Liu.
  - ID: ZL202111384841.3 (Authorized: Aug 2024)

- **[Patent] Log Anomaly Detection** based on LDA (Latent Dirichlet Allocation) topic features.
  - Inventors: Hua Dai, Xuekui Sun, Jianguo Zhou, **Qian Zhou**, Geng Yang, Yanli Chen.
  - ID: ZL202210689100.4 (Authorized: Nov 2024)

---

## II. Blockchain &amp; Trusted Privacy-Preserving Systems

Building secure frameworks for logistics, IoV (Internet of Vehicles), and smart agriculture.

- **[SW] Blockchain-based Contextual Location Privacy Protection System.**
  - A complete software suite for secure local delivery.
  - Developers: **Qian Zhou**, Jiayang Wu, Tianlong Zhang, Zhongxu Han. 
  - Copyright ID: 2018SR271600 (Ranking: 1st)

- **[Patent] Contextual Location Privacy Preservation** for express delivery based on blockchain.
  - Inventors: **Qian Zhou**, Tianlong Zhang, Jiayang Wu, Zhongxu Han.
  - ID: CN202111398742.0 (Authorized: Jun 2025)

- **[Patent] Anonymous Identity Authentication Scheme** for IoV based on improved group signatures.
  - Inventors: Yujuan Yang, **Qian Zhou**, Zhixin Sun, Zhe Sun.
  - ID: ZL202310234793.2 (Authorized: Apr 2023)

- **[Patent] Consensus Mechanism Operation Method** for Consortium Blockchains.
  - Inventors: Zhixin Sun, Furu Lv, Yuhua Xu, Zhe Sun, **Qian Zhou**, Bingqing Luo, Bo Song.
  - ID: ZL202010626701.1 (Authorized: Aug 2022)

- **[Patent] Data Monitoring &amp; Auto-control** in Smart Agriculture based on dual-chain mechanism.
  - Inventors: Zhixin Sun, Yuhua Xu, Huqing Wang, Bing Hu, Zhe Sun, **Qian Zhou**.
  - ID: ZL202010655335.2 (Authorized: Mar 2022)

---

## III. Domestic Autonomous Platforms &amp; Cloud Security

Dedicated to high-performance middleware and efficient encrypted data retrieval.

- **[SW] Distributed Message Middleware** with High Real-time &amp; Fault Tolerance for **Domestic Autonomous Computing Platforms**.
  - Developers: **Qian Zhou**, Jiayang Wu, Nanjing Savage Intelligent Technology Research Institute Co., Ltd.
  - Copyright ID: 2023SR0997588 (Ranking: 1st)

- **[Patent] Semantic-aware Feature Keyword Extraction** for optimized ciphertext retrieval in cloud environment.
  - Applicants: Hua Dai, Yingying Jiang, Xuelong Dai, **Qian Zhou**, Geng Yang, Haiping Huang.
  - ID: ZL2020092500303650 (Authorized: Oct 2022)

- **[Patent] Retrieval Filtering Threshold Selection** for cloud-based ciphertext search efficiency.
  - Applicants: Hua Dai, Yingying Jiang, Zhixiu Chen, Tianchang Yang, **Qian Zhou**, Geng Yang, Haiping Huang.
  - ID: ZL2020122202325270 (Authorized: Sept 2022)

---

## IV. Spatio-Temporal Data Mining &amp; Spreading Pattern Analysis

Developing intelligent algorithms for mobile objects and contagious behavior mining.

- **[Patent] Multi-source Contagious Event Discovery** for mobile objects.
  - Inventors: Hua Dai, Yu Chen, **Qian Zhou**, Yuanlong Liu, Geng Yang, Yanli Chen.
  - ID: ZL2022100925651 (Authorized: Nov 2024)

- **[Patent] Contagious Behavior Mining** based on binary filtering for mobile objects.
  - Applicants: Hua Dai, Yu Chen, Jiawei Li, **Qian Zhou**, Geng Yang, Haiping Huang.
  - ID: ZL2020122600529420 (Authorized: Feb 2023)

- **[Patent] Spreading Pattern Mining** based on trajectory data.
  - Applicants: Hua Dai, Tonghui Zhang, **Qian Zhou**, Jiawei Li, Geng Yang, Haiping Huang.
  - ID: ZL202010950084.0 (Authorized: Oct 2022)

- **[Patent] Geographic Coordinate Density Clustering** based on GeoHash.
  - Inventors: Hua Dai, Yiyang Liu, Yu Chen, Geng Yang, Jiawei Li, **Qian Zhou**.
  - ID: ZL202110644375.1 (Authorized: Jul 2022)
